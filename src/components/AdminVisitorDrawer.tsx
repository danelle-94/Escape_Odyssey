...
export const AdminVisitorDrawer: React.FC<AdminVisitorDrawerProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/60 backdrop-blur-sm animate-fadeIn">
      <div className="relative w-full max-w-lg bg-white border-l border-blue-200 h-full overflow-y-auto p-6 shadow-2xl flex flex-col justify-between text-slate-800">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between border-b border-slate-100 pb-4">
            <div className="flex items-center gap-2.5">
              <div className="w-10 h-10 rounded-xl bg-blue-50 border border-blue-200 flex items-center justify-center text-[#015da5] shadow-sm">
          ...
          <div className="bg-blue-50 p-4 rounded-xl border border-blue-200 space-y-2">
            <div className="flex items-center justify-between text-xs text-[#015da5] font-bold">
              <span className="flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4 text-[#015da5]" /> Anonymous Visitor Identifier
              </span>
              <button
                onClick={fetchFingerprint}
                disabled={loading}
                className="hover:text-[#01477f] flex items-center gap-1 transition-colors text-[10px]"
              >
                <RefreshCw className={`w-3 h-3 ${loading ? 'animate-spin' : ''}`} /> Refresh
              </button>
            </div>

            <div className="p-3 bg-white rounded-lg font-mono text-sm text-[#015da5] select-all break-all border border-blue-200 flex items-center justify-between shadow-sm">
            ...
            <p className="text-[11px] text-slate-600 font-medium">
              Generated client-side via official <code className="text-[#015da5] font-mono font-bold">@fingerprintjs/fingerprintjs</code> with <code className="text-[#015da5] font-mono font-bold">monitoring: false</code>. Never exposed in public UI or stored as secrets.
            </p>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
                <Activity className="w-4 h-4 text-[#015da5]" /> Recorded Visitor Journey ({events.length})
              </h4>
            ...
            {events.length === 0 ? (
              ...
              <div className="space-y-2.5 max-h-[50vh] overflow-y-auto pr-1">
                {events.slice().reverse().map((ev, idx) => (
                  <div key={idx} className="p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs space-y-1 hover:border-blue-300 transition-all">
                    <div className="flex items-center justify-between text-[#015da5] font-bold">
                      <span className="flex items-center gap-1">
                        <Layers className="w-3.5 h-3.5 text-[#015da5]" /> {ev.eventName}
                      </span>
                    ...
                    {(ev.destination || ev.visaType) && (
                      <div className="flex flex-wrap gap-2 pt-1 text-[10px]">
                        {ev.visaType && (
                          <span className="px-2 py-0.5 bg-blue-50 border border-blue-200 rounded text-[#015da5] font-bold">
                            Visa: {ev.visaType}
                          </span>
                        )}
                        {ev.destination && (
        ...
        <div className="pt-6 border-t border-slate-100 text-[11px] text-slate-500 text-center space-y-2 font-medium">
          ...
          <button
            onClick={onClose}
            className="w-full py-2.5 rounded-xl bg-blue-50 hover:bg-blue-100 text-[#015da5] font-bold text-xs transition-colors border border-blue-200"
          >
            Close Inspector
          </button>
        </div>
